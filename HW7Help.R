...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    radioButtons(
      "hhl_corr",
      "Household Language",
      choices=list("All", "English only","Spanish",
                   "Other"),
      selected="All"
    ),
    radioButtons(
    ...
    radioButtons(
      ...
      choices=list("All","High School not Complete",
...
server <- function(input, output,session) {
  ...
  observeEvent(input$corr_sample,{
    ...
    if(input$schl_corr == "All"){
      schl_sub <- SCHLvals
    } else if(input$schl_corr == "High School not Complete"){
    ...
    sample_corr<- reactiveValues(corr_data=subsetted_data[index,], 
                                 corr_truth=cor(sample_corr$corr_data|>select(corr_vars))[1,2])
    
    
    
    #Create a renderPlot() object to output a scatter plot
    output$scatter<-renderPlot({
      plot(sample_corr$corr_data)
    })
